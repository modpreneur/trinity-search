...
class DQLConverter
{
    ...
    private function fetchAvailableEntities() {
        ...
        foreach ($meta as $m) {
            ...
            if(StringUtils::startsWith(str_replace("\\","", $m->getName()), $this->bundleName) && !in_array($entityName, self::$ignoredEntities)) {
        ...
        foreach($conditions as $cond) {
            switch($cond->type) {
                ...
                case WherePartType::CONDITION:
                    $whereClause.=" ".(is_null($cond->key->getJoinWith()) ? ($cond->key->getAlias() ?? $columnDefaultAlias) : $cond->key->getJoinWith()).".".$cond->key->getName().$cond->operator."?".$paramCounter;
    ...
    private function getAllColumnsFromWhere($conditions) {
        ...
        foreach($conditions as $cond) {
            ...
            else if($cond->type == WherePartType::SUBCONDITION) {
